...
import { handleMarketingDemo } from '@/actions/api'
          ...
          <div className="flex gap-2">
            <div className="relative">
              <select
                ...
                className="border-input h-12 min-h-[48px] w-[120px] cursor-pointer appearance-none rounded-md border bg-white pr-8 pl-9"
              >
                {countries.map((country: Country) => (
                  <option
                    key={`${country.code}-${country.name}`}
                    value={country.code}
                    className="flex h-12 items-center gap-2 px-3 py-2"
                  >
                    {country.code}
                  </option>
                ))}
              </select>
              <div className="pointer-events-none absolute top-1/2 left-2 flex -translate-y-1/2 items-center">
