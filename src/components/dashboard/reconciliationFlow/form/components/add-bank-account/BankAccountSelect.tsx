...
const BankAccountSelect = ({ form }: BankAccountSelectProps) => {
  const {
    setValue,
    watch,
    formState: { errors },
  ...
  return (
    <Select
      ...
      onValueChange={(value) =>
        setValue('bankAccount', value === 'none' ? '' : value, { shouldValidate: true })
      }
    >
      <SelectTrigger
        hideIcon
        className={cn(
          'group hover:bg-accent dark:border-border dark:bg-background dark:text-foreground dark.data-[placeholder]:text-muted-foreground flex w-full cursor-pointer items-center justify-between border-black/20 placeholder:text-sm placeholder:font-medium data-[placeholder]:text-black data-[size=default]:h-10',
