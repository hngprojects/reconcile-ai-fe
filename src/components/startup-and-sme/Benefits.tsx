...
export default function Benefits() {
  return (
    <section
      className="bg-[#FCFCFC] px-4 py-8 sm:px-6 sm:py-10 lg:px-0"
    ...
    >
      <Container className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center">
          <h2
            id="benefits-title"
            className="mb-6 max-w-3xl text-center text-2xl leading-tight font-semibold tracking-tight text-[#101828] sm:mb-8 sm:text-3xl md:mb-12 md:text-4xl md:leading-[1.2]"
          >
            Benefits of Using ReconXi
          </h2>

          <div className="grid w-full max-w-6xl grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6">
            {benefits.map((benefit, index) => (
              <div
                key={index}
                className="flex flex-col rounded-lg bg-white px-4 py-4 duration-300 sm:px-5 sm:py-6"
                role="listitem"
              >
                <h3 className="mb-2 flex items-center gap-3 text-xl font-semibold text-[#2A2A2A] sm:mb-3 sm:text-2xl">
                  <span className="flex h-6 w-6 items-center justify-center">
                    {<benefit.icon />}
                  </span>
                  {benefit.title}
                </h3>
                <p className="text-base leading-relaxed font-normal text-[#3B3E45] sm:text-lg">
                  {benefit.description}
                </p>
              </div>
            ))}
