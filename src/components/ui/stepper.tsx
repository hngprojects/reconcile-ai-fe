...
// //#region Types
// type StepperVariant = 'horizontal' | 'vertical' | 'circle' | 'progress'
...
const defineStepper = <const Steps extends Stepperize.Step[]>(
  ...
  return {
    ...
    StepperStep: ({ children, className, icon, ...props }) => {
      ...
      // For Progress variant - calculate percentage for Shadcn Progress
      ...
      if (variant === 'progress') {
        return (
          <li
            className={cn(
              'stepper-step w-full rounded-md transition-colors',
              className
            )}
          >
            <div className="stepper-step-content flex w-full flex-col items-start gap-2">
              {title}
              ...
              <div className="flex w-full justify-between text-sm text-[#475467]">
                <span>
